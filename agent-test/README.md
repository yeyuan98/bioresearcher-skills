...
| `deep-research-q06-ledger-drill` | L2 | deterministic CLI feature drill: secondary-id dedupe + key promotion, quarantine, `[MISSING record]` non-zero exit, `--expand-pages`, epub rendering, Vancouver initials, name→title fold, render hard-fail contract, check + `--markers` marker gate | — | manual-run |
