...
| `deep-research-q04-evidence-ledger` | L2 | evidence-ledger light happy path: worker ledger written via the script, `evidence-ledger.py` check + dry-run verify + render run, tier-independent pmid-keys anchor + banners reported | MCP | manual-run |
| `deep-research-q05-ledger-merge` | L3 | Step 5 pipeline end-to-end: per-aspect ledgers merge → verify --apply → render, References generated by render; checks anchored on ledger stdout (dispatch-proof) | MCP | manual-run |
...
| `deep-research-q07-citation-fidelity` | L3 | citation-fidelity regression: References generated by `render` (by construction) + vet structural audit PASS + re-render diff `MISMATCH COUNT: 0` (no post-render hand edits) + quantitative-claims spot-check | MCP | manual-run |
