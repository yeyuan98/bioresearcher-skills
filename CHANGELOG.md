...
- Orchestrator completion gate: `SKILL.md` Step 4 adds explicit `evidence-ledger.py check` execution per aspect upon worker return, formalizes the Remediation Worker protocol and prompt template with serialized ownership, and enforces a deterministic 2-retry circuit breaker.
...
- Prompt budget revamp: streamlined redundant interview/autonomy prose in `SKILL.md`, reducing prompt size from 498 lines to 403 lines (comfortably below the 500-line CI limit).
