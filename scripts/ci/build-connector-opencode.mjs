#!/usr/bin/env node
// OpenCode connector/plugin bundle builder. Zero deps.
// Stages connector/opencode/ + bundled skills + dr-worker agent into <out>/bioresearcher/,
// validates the whole bundle, and writes a reproducible
// bioresearcher-connector_opencode-v<VERSION>.tar.gz next to the staging dir.
// Repo sources are never modified.
//
// Usage: node scripts/ci/build-connector-opencode.mjs [--out DIR]   (DIR defaults to dist/)
...
const FLAVOR = join(ROOT, "connector", "opencode");
let failures = 0;
...
// --- reproducible tar ----------------------------------------------------------
const tarball = join(stageRoot, `bioresearcher-connector_opencode-v${version}.tar.gz`);
...
ok(`tarball ${join(outDir, `bioresearcher-connector_opencode-v${version}.tar.gz`)}`);
